...
User_Password = input("Your naw password: ")
port = None
Sudo = input("Your naw sudo password: ")
while True:
    if port != None:
        break
    else:
...
try:
    ...
    def Sudo_user(message):
        bot.send_message(message.from_user.id, "🔐Ведите Sudo🔐")
        bot.register_next_step_handler(message, close_Password)
    ...
    def password(message):
        ...
        elif message.text != User_Password:
            for User in UserTrue:
                if User == message.from_user.id:
                    UserFalse = False
                    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    ...
    def close_Password(message):
        if message.text == Sudo:
            bot.send_message(message.from_user.id, "🔏Ведите New Sudo🔏")
            bot.register_next_step_handler(message, new_Password)
        else:
            bot.send_message(message.from_user.id, "⚠️Sudo не верный⚠️")
            password(message)
    ...
    def new_Password(message):
        ...
        UserTrue = []
        bot.send_message(message.from_user.id, "🔏Пароль успешно сменен!🔏")
        password(message)
